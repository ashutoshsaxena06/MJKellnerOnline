package com.market.ashent.MJKellnerOnline;
...
import com.util.framework.CommonMJ;
...
public class TestMJ extends CommonMJ {

	// public testMJ(WebDriver driver) {
	// super(driver);
	// }
	...
	public static String project = "Others";
	private final static Logger logger = Logger.getLogger(TestMJ.class);
	public static CommonMJ testMJ = new TestMJ();
	...
	public static void Export_Mail_OG(String active, String accountID, String purveyor, String restaurant_name,
		...
		TestMJ.rowIndex = Math.floorMod(TestMJ.acno, TestMJ.totalNoOfRows) + 1;

		System.out.println("Test Case test #" + TestMJ.rowIndex);

		cell1 = TestMJ.exportworkbook.getSheet(project).getRow(TestMJ.rowIndex).createCell(TestMJ.AcColStatus);
		cell1.setCellValue("");

		cell2 = TestMJ.exportworkbook.getSheet(project).getRow(TestMJ.rowIndex).createCell(TestMJ.AcColdetail);
		cell2.setCellValue("");
		...
		try {
			if (active.equalsIgnoreCase("Yes")) {
				if (purveyor.equalsIgnoreCase("MJ Kellner")) {
					// if list is not empty
					System.out.println(restaurant_name + " for purveryor " + purveyor + " is Active !!");
					if (listname != null && listname.length() != 0) {
						result = testMJ.startPFG(listname.trim(), username.trim(), password.trim());
					...
					System.out.println("No Restaurant available for MJ Kellner");
				}
			} else {
